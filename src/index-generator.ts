/**
 * @file Conversation index generator for trace log directories.
 *
 * Scans JSONL session logs, optionally regenerates per-log HTML reports and
 * AI-generated conversation summaries via the Claude CLI, then writes a
 * browsable `index.html` listing all sessions and their summaries.
 */
...
/** One summarized conversation extracted from a session log. */
export interface ConversationSummary {
	/** Stable id matching the merged {@link SimpleConversation} id. */
	id: string;
	/** Short title generated by the Claude CLI summarizer. */
	title: string;
	/** One-to-three paragraph summary from the CLI. */
	summary: string;
	/** ISO timestamp of the conversation start. */
	startTime: string;
	/** Number of user/assistant turns in the thread. */
	messageCount: number;
	/** Distinct model ids used across the thread's API pairs. */
	models: string[];
}

/** Cached summary metadata for a single JSONL log file. */
export interface LogSummary {
	/** Source JSONL filename, e.g. `log-2026-06-10-12-00-00.jsonl`. */
	logFile: string;
	/** Companion HTML report filename in the same directory. */
	htmlFile: string;
	/** ISO timestamp when this sidecar was last written. */
	generated: string;
	/** Per-conversation summaries for index.html rendering. */
	conversations: ConversationSummary[];
}

/**
 * Builds and maintains `index.html` plus per-log `summary-*.json` sidecars.
 *
 * Summaries are regenerated when missing or older than the source JSONL file.
 */
export class IndexGenerator {
	...
	/**
	 * @param traceDir - Trace output directory (e.g. `.claude-trace`).
	 */
	...
	/**
	 * Main entry point: process all logs in `traceDir` and write `index.html`.
	 * Exits the process with code 1 when the directory or logs are missing.
	 */
	...
	/**
	 * Lists session JSONL files matching `log-YYYY-MM-DD-HH-MM-SS.jsonl`,
	 * newest first.
	 *
	 * @returns Filenames only (not full paths), sorted descending by timestamp.
	 */
	...
	/**
	 * Ensures a summary sidecar exists for one log, regenerating when stale.
	 * Also generates the per-log HTML report if missing.
	 */
	...
	/** Parses a JSONL log into merged {@link SimpleConversation} objects. */
	...
	/**
	 * Calls the Claude CLI to produce a title and summary for one conversation.
	 * Returns null when parsing fails or the CLI is unavailable.
	 */
...
SUMMARY: [summary]`;
	...
	/**
	 * Flattens a conversation to plain text for the summarization prompt.
	 * Truncates long blocks to keep token usage reasonable.
	 */
	...
	/**
	 * Spawns `claude -p <prompt>` and returns stdout.
	 * @throws When the CLI exits non-zero or cannot be spawned.
	 */
	...
	/**
	 * Writes a static HTML index page linking to per-log reports and summaries.
	 *
	 * Uses inline CSS only — no frontend bundle — so the index opens instantly
	 * without rebuilding the Lit viewer.
	 */
	private async generateIndexHTML(summaries: LogSummary[]): Promise<void> {
		// Static template; a future version may reuse the lazy-loading frontend bundle
